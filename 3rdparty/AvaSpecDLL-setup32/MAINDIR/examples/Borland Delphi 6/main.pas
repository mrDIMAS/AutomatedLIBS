...
uses
  ...
  Dialogs, avaspec, Menus, StdCtrls, TeeProcs, Chart, Series, ExtCtrls,
  Grids, ComCtrls, u_Usb, TeEngine;
...
begin
  ...
  begin
    ...
    if (StrToInt(edtNrOfScans.Text)>0) then // finite nr of measurements
       btnStartMeasurement.Enabled:=(StrToInt(edtNrOfScans.Text)= m_Measurements); //enable startbutton when ready
  end
...
begin
  ...
  begin
    AVS_Done();
    l_Port:=AVS_Init(-1);// try Ethernet/USB
    if (l_Port > 0) then
    begin
...
var l_Active: AvsIdentityType;
    l_hDevice: AvsHandle;
begin
  StrPCopy(l_Active.SerialNumber,sgShowList.Cells[0,sgShowList.Selection.Top]);
...
var l_Res: longint;
    ...
    l_FPGAver,
    l_FWver,
    l_DLLver:array[0..15] of char;
begin
  if (ERR_SUCCESS = AVS_GetVersionInfo(m_DeviceHandle, l_FPGAver, l_FWver, l_DLLver)) then
  begin
    edtFPGAver.Text:=l_FPGAver;
    edtFWver.Text:=l_FWver;
    edtDLLver.Text:=l_DLLver;
  end;
...
begin
  ...
  begin
    ...
    l_pDeviceData.m_StandAlone.m_Meas.m_Control.m_StoreToRam:=StrToInt(edtRamSpectra.Text);
    l_pDeviceData.m_StandAlone.m_Nmsr:= StrToInt(edtNrOfScans.Text);
    // Write parameters
