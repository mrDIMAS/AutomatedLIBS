...
void __fastcall TMainForm::btnCommunicationClick(TObject *Sender)
{
    ...
    {
        AVS_Done();
        l_Port = AVS_Init(-1);   // try Ethernet/USB setting
        if (l_Port > 0)
        {
...
void __fastcall TMainForm::FormClose(TObject *Sender, TCloseAction &Action)
{
    if (m_DeviceHandle!=INVALID_AVS_HANDLE_VALUE)
    {
      ...
      {
          char buf[80];
          wsprintf(buf, "Error in AVS_StopMeasure, code: %d", l_Res);
    ...
    if ( a_WinMess.WParam >= ERR_SUCCESS)
    {
        ...
        if (m_NrPixels > 0)
        {
            ...
            if ( a_WinMess.WParam == ERR_SUCCESS)  //normal measurements
            {
               ...
               edtTotalTime->Text =  IntToStr((l_Ticks - m_StartTicks)/ 1000);
...
void __fastcall TMainForm::btnActivateClick(TObject *Sender)
{
    ...
    strcpy(l_Active.SerialNumber, sgShowList->Cells[0][sgShowList->Selection.Top].c_str());
