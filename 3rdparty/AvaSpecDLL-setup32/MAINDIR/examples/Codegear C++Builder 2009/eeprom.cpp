...
void __fastcall TfrmEEprom::FormShow(TObject *Sender)
{
	...
	edtIrradFiberDiameter->Text = IntToStr(int(l_pDeviceData->m_Irradiance.m_FiberDiameter));
...
void __fastcall TfrmEEprom::btnSaveToEEPromClick(TObject *Sender)
{
	...
	AnsiString s = edtUserFriendly->Text;
	l_len = s.Length();
	for (j = 0; j < l_len; j++)
	{
		l_pDeviceData->m_aUserFriendlyId[j] = s[j+1];
	}
